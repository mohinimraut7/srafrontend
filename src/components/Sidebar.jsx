...
//     { id: "getUser", label: "Users", icon: User },
   
 
  
   
//     // { id: "hut-master", label: "Hut Master", icon: Edit3 },
//     // { id: "ownershipofhut", label: "Ownership Of Hut", icon: Edit3 },
//   ]
...
const Sidebar = ({ isCollapsed, currentPage, currentMode, onNavigate }) => {
  ...
  // 🔹 Role Based Filtering (Only Surveyor Restriction)
  const filteredNavigationItems =
    role === "surveyor"
      ? navigationItems.filter(
          (item) =>
            item.id !== "cluster-master" &&
            item.id !== "slum-master" &&
            item.id !== "getUser"
        )
      : navigationItems
