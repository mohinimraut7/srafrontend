...
//   // Date range state — for Download Excel dropdown
...
//     return usersMap[String(submittedBy)] || submittedBy
//   }

//   // Build ALL fields Excel row
//   const buildExcelRow = (app, index) => {
//     const hutIdDisplay = app.cluster_number && app.hut_id
//       ? `${app.cluster_number}${String(app.hut_id).padStart(5, "0")}`
//       : app.hut_id || ""
//     const row = {}
//     row["Serial No"] = index + 1
//     row["Hut ID (Display)"] = hutIdDisplay
//     row["Hut Number"] = app.hut_id || ""
//     row["Done By"] = getUserName(app.submittedBy)
//     Object.entries(app).forEach(([key, value]) => {
//       const label = key.replace(/_/g, " ").replace(/([A-Z])/g, " $1")
//         .split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ").trim()
//       row[label] = value !== null && value !== undefined ? value.toString() : ""
//     })
//     return row
//   }

//   // Parse created_date to JS Date (handles DD-MM-YYYY and YYYY-MM-DD)
...
//   // Download Excel — direct click = all records, from date picker = filtered by range
//   const handleDownloadExcel = (from, to) => {
//     let appsToExport = filteredApplications
//     if (from || to) {
//       appsToExport = filteredApplications.filter((app) => {
//         const d = parseAppDate(app.created_date)
...
//       })
//     }
//     if (appsToExport.length === 0) { alert("No data available!"); return }
//     const excelData = appsToExport.map((app, i) => buildExcelRow(app, i))
//     const ws = XLSX.utils.json_to_sheet(excelData, { origin: "A4" })
//     XLSX.utils.sheet_add_aoa(ws, [["BMC Applications Report"]], { origin: "A1" })
//     const label = (from || to) ? ` | ${from || "start"} to ${to || "end"}` : ""
//     XLSX.utils.sheet_add_aoa(ws, [[`Total Records: ${appsToExport.length}${label}`]], { origin: "A2" })
//     const wb = XLSX.utils.book_new()
//     XLSX.utils.book_append_sheet(wb, ws, "Applications")
//     XLSX.writeFile(wb, `BMC_Applications${label ? "_Filtered" : ""}.xlsx`)
//     setShowDatePicker(false)
//   }

//   // Single row Excel download
//   const handleDownloadSingleExcel = (app) => {
//     const row = buildExcelRow(app, 1)
//     const ws = XLSX.utils.json_to_sheet([row], { origin: "A2" })
//     XLSX.utils.sheet_add_aoa(ws, [[`Application: ${app.first_name || ""} ${app.last_name || ""}`]], { origin: "A1" })
//     const wb = XLSX.utils.book_new()
//     XLSX.utils.book_append_sheet(wb, ws, "Application")
//     XLSX.writeFile(wb, `Application_${app.hut_id || app.id}_${app.first_name || ""}_${app.last_name || ""}.xlsx`)
//   }
...
//         data.forEach((u) => {
//           const name = [u.first_name, u.last_name].filter(Boolean).join(" ") || u.name || u.username || u.email || String(u.id)
...
//         const st = (app.survey_status || "").toLowerCase()
//         if (!st.includes(surveyStatusFilter.toLowerCase())) return false
//       }
//       return true
//     })
...
//       {/* Header Buttons — Download Excel: direct=all, dropdown=date range */}
//       <div className="mb-9 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
//         <h2 style={{ color: '#4A5565', textTransform: 'uppercase' }} className="text-2xl font-bold tracking-wide">BMC Applications</h2>
//         <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">

//           {/* Download Excel button — direct click downloads ALL, arrow opens date picker */}
//           <div className="relative" ref={datePickerRef}>
//             <div className="flex rounded-xl overflow-hidden shadow-md">
//               {/* Direct download — all records */}
//               <button
//                 onClick={() => handleDownloadExcel("", "")}
//                 className="flex items-center gap-2 px-5 py-3 font-semibold text-white bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 transition-all duration-300 hover:-translate-y-1 active:scale-95"
//               >
//                 <Download size={18} />Download Excel
//               </button>
//               {/* Small arrow to open date range */}
//               <button
//                 onClick={() => setShowDatePicker(v => !v)}
//                 className="px-2 py-3 text-white bg-green-600 hover:bg-green-700 border-l border-green-400 transition-all duration-200"
//                 title="Filter by date range"
//               >
//                 <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 4.5L7 9.5L12 4.5" stroke="white" strokeWidth="2" strokeLinecap="round"/></svg>
//               </button>
//             </div>
//             {/* Date range dropdown */}
//             {showDatePicker && (
//               <div className="absolute right-0 top-full mt-2 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 w-64">
//                 <p className="text-sm font-semibold text-gray-700 mb-3">Download by Date Range</p>
//                 <div className="space-y-2 mb-3">
//                   <div>
//                     <label className="block text-xs text-gray-500 mb-1">From</label>
//                     <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:outline-none" />
//                   </div>
//                   <div>
//                     <label className="block text-xs text-gray-500 mb-1">To</label>
//                     <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:outline-none" />
//                   </div>
//                 </div>
//                 <div className="flex gap-2">
//                   <button
//                     onClick={() => handleDownloadExcel(dateFrom, dateTo)}
...
//         <div className="relative mb-6">
//           <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><Search className="text-gray-500" size={18} /></div>
//           <input type="text" placeholder="Search by name, Hut ID, Hut Number, cluster number, slum ID, or Aadhaar..." className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-400 transition-all duration-200 text-sm bg-gray-50" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1) }} />
//         </div>
//         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
//           <div>
//             <label className="block text-sm font-semibold text-gray-700 mb-2">Filter by Use of Hut</label>
...
//       {/* Table — py-4 → py-2 ONLY in tbody rows */}
//       <div className="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
//         <div className="overflow-x-auto">
//           <table className="w-full text-sm text-left">
//             <thead className="bg-gradient-to-r from-orange-100 to-orange-50 text-[#2D3748] uppercase tracking-wider font-bold text-xs">
...
//             <tbody className="divide-y divide-gray-200">
//               {paginatedApplications.length === 0 ? (
//                 <tr><td colSpan="13" className="px-6 py-12 text-center text-gray-500">No applications found matching your search criteria</td></tr>
//               ) : (
//                 paginatedApplications.map((app, index) => {
//                   const hutIdDisplay = app.cluster_number && app.hut_id
//                     ? `${app.cluster_number}${String(app.hut_id).padStart(5, "0")}`
...
//                   return (
//                     <tr key={app.id} className="odd:bg-white even:bg-gray-50 hover:bg-orange-50 transition-all duration-200">
//                       <td className="px-6 py-2 font-medium text-gray-700">{(currentPage - 1) * itemsPerPage + index + 1}</td>
//                       <td className="px-6 py-2 text-gray-700">{app.cluster_number || "N/A"}</td>
//                       <td className="px-6 py-2 text-gray-700">{hutIdDisplay}</td>
//                       <td className="px-6 py-2 text-gray-700">{app.hut_id || "N/A"}</td>
//                       <td className="px-6 py-2">
//                         <div className="font-semibold text-gray-800 truncate">{app.first_name} {app.middle_name && `${app.middle_name} `}{app.last_name}</div>
//                       </td>
//                       <td className="px-6 py-2 text-gray-600"><div className="text-xs">{app.current_pincode && `PIN: ${app.current_pincode}`}</div></td>
//                       <td className="px-6 py-2 text-center">
//                         {app.slum_floor === "G" ? (<span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-green-100 text-green-700 font-semibold shadow-sm">{app.slum_floor}</span>) : (<span className="text-gray-700">{app.slum_floor || "N/A"}</span>)}
//                       </td>
//                       <td className="px-6 py-2 text-gray-700">{app.slum_use || "N/A"}</td>
//                       <td className="px-6 py-2">
//                         <div className="font-medium text-gray-800">{app.area_sq_m}</div>
//                         <div className="text-gray-500 text-xs">{app.length && app.width && `${app.length}×${app.width}m`}</div>
//                       </td>
//                       <td className="px-6 py-2 text-gray-700">{app.created_date || "N/A"}</td>
//                       <td className="px-6 py-2 text-gray-700">{getUserName(app.submittedBy)}</td>
//                       <td className="px-6 py-2"><span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full shadow-sm ${getStatusColor(app.survey_status)}`}>{app.survey_status || "Pending"}</span></td>
//                       {role !== "surveyor" && (
//                         <td className="px-6 py-2 text-center">
//                           <div className="flex items-center justify-center gap-3">
//                             <button onClick={() => openModal(app)} className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition" title="View Details"><Eye size={18} /></button>
...
//   // Download Excel — direct click = all records, from date picker = filtered by range
//   const handleDownloadExcel = (from, to) => {
//     // Date range = ALL applications (pagination/search/filter ignore होते)
//     // Direct download (no date range) = current filteredApplications
//     let appsToExport
//     if (from || to) {
...
//       })
//     } else {
//       appsToExport = filteredApplications
//     }
//     if (appsToExport.length === 0) { alert("No data available!"); return }
//     const excelData = appsToExport.map((app, i) => buildExcelRow(app, i))
//     const ws = XLSX.utils.json_to_sheet(excelData, { origin: "A4" })
...
//   const normalizeCluster = (cn) => {
//   if (!cn) return ""
//   const lower = cn.toLowerCase()
//   const parts = lower.split("_")
//   // PNH_02600682 → parts = ["pnh", "02600682"] → "pnh_026"
//   // PNH_026 → parts = ["pnh", "026"] → "pnh_026"
//   if (parts.length < 2) return lower
//   return parts[0] + "_" + parts[1].substring(0, 3)
// }
...
//     } else {
//       // appsToExport = filteredApplications
//       appsToExport = [...applications].sort((a, b) => {
//         // const aCluster = (a.cluster_number || "").toLowerCase()
//         // const bCluster = (b.cluster_number || "").toLowerCase()
//  // हे add करा - एकदम पहिल्या line ला
  

//      if (a.id === 3546 || b.id === 3546) {
//     const u = a.id === 3546 ? a : b
//     console.log('UNDE IN SORT:', u.id, u.cluster_number, u.hut_id)
//   }

//       const aCluster = normalizeCluster(a.cluster_number)  // हे बदला
//       const bCluster = normalizeCluster(b.cluster_number)  // हे बदला
//         if (aCluster < bCluster) return -1
//         if (aCluster > bCluster) return 1
//         const aId = a.hut_id !== null && a.hut_id !== undefined && a.hut_id !== ""
//           ? Number(a.hut_id) : Infinity
//         const bId = b.hut_id !== null && b.hut_id !== undefined && b.hut_id !== ""
//           ? Number(b.hut_id) : Infinity
//         return aId - bId
//       })
...
//       if (Array.isArray(data)) {
//         data.forEach((u) => {
//           const name = u.name || [u.first_name, u.last_name].filter(Boolean).join(" ") || u.username || u.email || String(u.id)
//           map[String(u.id)] = name
//         })
//       }
//       setUsersMap(map)
...
//     ? `${app.cluster_number}${String(app.hut_id).padStart(5, "0")}`
//     : String(app.hut_id || "")
//   // underscore काढून plain version पण बनवा
//   const hutIdDisplayPlain = hutIdDisplay.replace(/_/g, "").toLowerCase()
//   const hutIdDisplayLower = hutIdDisplay.toLowerCase()
//   const clusterLower = (app.cluster_number || "").toLowerCase()
//   const clusterPlain = clusterLower.replace(/_/g, "")
//   // const clusterStart = clusterLower.substring(0, 3)
//   const clusterStart = clusterLower.split("_")[0]  // "pnh_026" → "pnh"
...
//   const searchTokens = s.trim().split(/\s+/).filter(Boolean)
//   return searchTokens.every(token => {
//     const tokenPlain = token.replace(/_/g, "")
//     return (
//       fullName.includes(token) ||
//       (app.slum_id && app.slum_id.toLowerCase().includes(token)) ||
//       (app.aadhaar_number && app.aadhaar_number.includes(token)) ||
//       clusterLower.includes(token) ||
//       clusterStart.includes(token) ||

//       clusterPlain.includes(tokenPlain) ||
//       (app.slum_use && app.slum_use.toLowerCase().includes(token)) ||
//       (app.hut_id && String(app.hut_id).toLowerCase().includes(token)) ||
//       hutIdDisplayLower.includes(token) ||
//       hutIdDisplayPlain.includes(tokenPlain)
//     )
//   })
// })
...
// // .sort((a, b) => {
// //   const aCluster = normalizeCluster(a.cluster_number)
// //   const bCluster = normalizeCluster(b.cluster_number)
// // // तात्पुरता debug
// //   if (a.id === 3546 || b.id === 3546) {
// //     console.log('UNDE sort:', {
// //       aId: a.id, aCluster, aHutId: a.hut_id,
// //       bId: b.id, bCluster, bHutId: b.hut_id
// //     })
// //   }
...
//    if (a.id === 3546 || b.id === 3546) {
//     const u = a.id === 3546 ? a : b
//     console.log('UNDE IN MAIN SORT:', u.id, u.cluster_number, u.hut_id)
//   }
//   const aCluster = normalizeCluster(a.cluster_number)
//   const bCluster = normalizeCluster(b.cluster_number)
//   if (aCluster < bCluster) return -1
...
//   // Debug: 680 आणि 709 का consecutive येतात?
//   if ((aId === 680 || bId === 680) && aCluster === 'pnh_026') {
//     console.log('680 compare:', {aId, bId, aCluster, bCluster})
//   }
//   if ((aId === 709 || bId === 709) && aCluster === 'pnh_026') {
//     console.log('709 compare:', {aId, bId, aCluster, bCluster})
//   }
...
//   const filteredApplications = applications
//     .filter((app) => { if (role === "surveyor") return Number(app.submittedBy) === Number(user_id); return true })
//     // ✅ CHANGE 2: slum_id search remove केला, cluster search improved
//     .filter((app) => {
//       const s = searchTerm.toLowerCase()
//       if (searchTerm === "") return true
//       const hutIdDisplay = app.cluster_number && app.hut_id
...
//       const clusterLower = (app.cluster_number || "").toLowerCase()
//       const clusterPlain = clusterLower.replace(/_/g, "")
//       // cluster prefix match: "PE_01" → "pe_01", "pe", "pe01" सगळे match होतील
//       const clusterStart = clusterLower.split("_")[0]
...
//           fullName.includes(token) ||
//           (app.aadhaar_number && app.aadhaar_number.includes(token)) ||
//           clusterLower.includes(token) ||         // "PE_01" exact match
//           clusterStart.includes(token) ||          // "pe" prefix match
//           clusterPlain.includes(tokenPlain) ||     // "pe01" without underscore
//           (app.slum_use && app.slum_use.toLowerCase().includes(token)) ||
//           (app.hut_id && String(app.hut_id).toLowerCase().includes(token)) ||
//           hutIdDisplayLower.includes(token) ||
//           hutIdDisplayPlain.includes(tokenPlain)
...
//       // hut_id numeric sort — "001","01","1","4","10" सगळे बरोबर serial order
//       const aHut = a.hut_id !== null && a.hut_id !== undefined && a.hut_id !== ""
//         ? String(a.hut_id) : null
//       const bHut = b.hut_id !== null && b.hut_id !== undefined && b.hut_id !== ""
//         ? String(b.hut_id) : null

//       if (aHut === null && bHut === null) return 0
//       if (aHut === null) return 1
//       if (bHut === null) return -1

//       return aHut.localeCompare(bHut, undefined, { numeric: true, sensitivity: 'base' })
//     })
...
//         <div className="relative mb-6">
//           <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><Search className="text-gray-500" size={18} /></div>
//           {/* ✅ CHANGE 2: Placeholder मधून "slum ID" काढला */}
//           <input type="text" placeholder="Search by name, Hut ID, Hut Number, cluster number, or Aadhaar..." className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-400 transition-all duration-200 text-sm bg-gray-50" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1) }} />
//         </div>
//         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
//           <div>
//             <label className="block text-sm font-semibold text-gray-700 mb-2">Filter by Use of Hut</label>
...
const AllApplicationsPage = () => {
  ...
  const handleDownloadExcel = (from, to) => {
    let appsToExport
    if (from || to) {
      // Date range filter — applications मधून date filter
      appsToExport = applications.filter((app) => {
        const d = parseAppDate(app.created_date)
        if (!d || isNaN(d)) return false
        if (from && d < new Date(from)) return false
      ...
      // No date range — जे screen वर दिसतंय तेच (filteredApplications)
      appsToExport = filteredApplications
    }
    if (appsToExport.length === 0) { alert("No data available!"); return }
    const excelData = appsToExport.map((app, i) => buildExcelRow(app, i))
