...
const isHutApposedOrDenied = (surveyStatus) =>
  surveyStatus === "Hut Appose" || surveyStatus === "Hut Denied"

// ✅ CHANGE 2: Step 1 schema — cluster_number + hut_id mandatory
const getStep1Schema = () =>
  Yup.object({
    cluster_number: Yup.string().required('Cluster number is required'),
    hut_id: Yup.string().required('Hut number is required'),
  })

// ✅ CHANGE 3: Step 2 schema — dynamic based on survey_status
const getStep2Schema = (surveyStatus) => {
  if (isHutApposedOrDenied(surveyStatus)) {
    // Hut Appose/Denied → owner details optional
    return Yup.object({
      aadhaar_mobile_number: Yup.string()
        .matches(/^[6-9]\d{9}$/, 'Enter a valid 10-digit mobile number')
  ...
  // Normal → owner details mandatory + aadhaar valid
  return Yup.object({
    first_name: Yup.string().matches(/^[A-Za-z\s]+$/, "Only alphabets are allowed").required('First name is required'),
    middle_name: Yup.string().matches(/^[A-Za-z\s]+$/, "Only alphabets are allowed").required('Middle name is required'),
    last_name: Yup.string().matches(/^[A-Za-z\s]+$/, "Only alphabets are allowed").required('Last name is required'),
...
const validationSchemas = {
  // step 1 & 2 handled dynamically via getStep1Schema / getStep2Schema
  3: Yup.object({
    current_address: Yup.string().required('Current address is required'),
    current_mobile_number: Yup.string()
     .matches(/^[6-9]\d{9}$/, 'Enter a valid 10-digit mobile number')
    ...
    voter_card_number: Yup.string().when("voter_card_type", (type, schema) => {
      if (type === "EPIC 10 Digit") {
        return schema.matches(/^[A-Z0-9]{10}$/, "Voter card number must be exactly 10 characters")
      }
      if (type === "EPIC 14 Digit") {
        return schema.matches(/^[A-Z0-9]{14}$/, "Voter card number must be exactly 14 characters")
      }
      return schema;
    })
  }),
...
const handleFileChange = (e) => {
  ...
  // ✅ CHANGE 4: nextStep — dynamic schema for step 1 & 2
  const nextStep = (formik) => {
    let schemaToValidate

    if (currentStep === 1) {
      schemaToValidate = getStep1Schema()
    } else if (currentStep === 2) {
      schemaToValidate = getStep2Schema(formik.values.survey_status)
    } else {
      schemaToValidate = validationSchemas[currentStep]
    }

    if (schemaToValidate) {
      schemaToValidate.validate(formik.values, { abortEarly: false })
        .then(() => {
          if (currentStep < steps.length) {
            setCurrentStep(currentStep + 1)
            if (currentStep + 1 === 3) setTimeout(() => autoFetchLocation(formik), 500)
    ...
    switch (currentStep) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">महानगरपालिकेचे नाव *</label>
                <Field type="text" name="municipal_corporation" readOnly className="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">प्रभाग *</label>
                <Field type="text" name="ward" readOnly className="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">जिल्हा *</label>
                <Field type="text" name="district" readOnly className="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">तालुका *</label>
                <Field type="text" name="taluka" readOnly className="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">गाव *</label>
                <Field type="text" name="village" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">झोपडी क्रमांक *</label>
                <Field type="text" name="hut_id" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                <ErrorMessage name="hut_id" component="div" className="text-red-500 text-sm mt-1 font-medium" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">झोपडीचा मजला *</label>
                <Field as="select" name="slum_floor" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">झोपडीचा मजला निवडा</option>
                  <option value="G">G</option>
                  <option value="G+1">G+1</option>
                  <option value="G+2">G+2</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">झोपडीचा वापर *</label>
                <Field as="select" name="slum_use" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">झोपडीचा वापर निवडा</option>
                  <option value="Residential">Residential</option>
                  <option value="Commercial">Commercial</option>
                  <option value="Combine">Combine</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">झोपडीच्या जमिनीचे मालकी हक्क *</label>
                <Field as="select" name="ownership_of_slum_land" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">झोपडीच्या जमिनीचे मालकी हक्क निवडा</option>
                  <option value="State Government">State Government</option>
                  <option value="Central Government">Central Government</option>
                  <option value="Municipal Corporation">Municipal Corporation</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">सर्वेक्षण स्थिती निवडा *</label>
                <Field as="select" name="survey_status" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">सर्वेक्षण स्थिती</option>
                  <option value="Hut Appose">Hut Appose</option>
                  <option value="Hut Denied">Hut Denied</option>
                  <option value="readytosurvey">Ready To Survey</option>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Owner Details</h3>
            {/* ✅ Info banner for Hut Appose/Denied */}
            {isHutApposedOrDenied(formik.values.survey_status) && (
              <div className="bg-yellow-50 border border-yellow-300 text-yellow-800 px-4 py-3 rounded-lg text-sm font-medium">
                ⚠️ Hut Appose / Hut Denied असल्यामुळे Owner Details optional आहेत.
              </div>
            )}
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
              ...
              <div>
                {/* ✅ aadhaar_number mandatory label */}
                <label className="block text-sm font-medium text-gray-700 mb-2">आधार क्रमांक (१२ अंक) *</label>
                <Field type="text" name="aadhaar_number" maxLength="12" placeholder="123456789012" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                <ErrorMessage name="aadhaar_number" component="div" className="text-red-500 text-sm mt-1 font-medium" />
              </div>
              ...
              <div>
                {/* ✅ CHANGE 5: * removed from spouse_name */}
                <label className="block text-sm font-medium text-gray-700 mb-2">पत्नीचे / पतीचे नाव</label>
                <Field type="text" name="spouse_name" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                {/* ✅ CHANGE 6: * removed from user_email */}
                <label className="block text-sm font-medium text-gray-700 mb-2">वापरकर्त्याचा ईमेल</label>
                <Field type="email" name="user_email" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                <ErrorMessage name="user_email" component="div" className="text-red-500 text-sm mt-1 font-medium" />
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                <div>
                  ...
                  <Field
                    ...
                    rows="3"
                    readOnly={formik.values.same_as_aadhaar}
                    className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 ${formik.values.same_as_aadhaar ? "bg-gray-100 cursor-not-allowed" : ""}`}
                  />
                ...
                <div className="flex items-center mb-3">
                  <Field
                    ...
                    className="h-4 w-4 text-blue-600"
                  />
                  <label className="ml-2 text-sm text-gray-700">आधार पत्ता आणि सध्याचा पत्ता एकच आहे</label>
                </div>
              </div>
              ...
              <div>
                ...
                <Field
                  ...
                  placeholder="400001"
                  readOnly={formik.values.same_pincode_as_aadhaar}
                  className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 ${formik.values.same_pincode_as_aadhaar ? "bg-gray-100 cursor-not-allowed" : ""}`}
                />
                <div className="flex items-center mt-2">
                  <input
                    type="checkbox"
                    ...
                    className="h-4 w-4 text-blue-600"
                  />
                  <label className="ml-2 text-sm text-gray-700">आधार पिनकोड आणि सध्याचा पिनकोड एकच आहे</label>
                </div>
                <ErrorMessage name="current_pincode" component="div" className="text-red-500 text-sm mt-1 font-medium" />
              </div>
              ...
              {formik.values.voter_card_type && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    मतदार ओळख क्रमांक ({formik.values.voter_card_type === "EPIC 10 Digit" ? "१०" : "१४"} अंक) *
                  </label>
                  <Field name="voter_card_number">
                    {({ field }) => (
                      <input
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <button
                  ...
                  onClick={() => fetchCurrentLocation(formik)}
                  disabled={fetchingLocation}
                  className={`w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-3 rounded-lg font-semibold transition-all ${fetchingLocation ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg'} text-white`}
                >
                  {fetchingLocation ? <>Fetching...</> : <><Crosshair size={20} /> Get Current Location</>}
                </button>
                {formik.values.biometric_lat && formik.values.biometric_long && (
                  <div className="w-full sm:w-auto flex items-center gap-2 px-4 py-2 bg-green-100 border border-green-300 rounded-lg">
                    <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">लांबी (मीटर) *</label>
                  <Field
                    type="number" step="0.1" name="length"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    onChange={(e) => {
                      formik.setFieldValue('length', e.target.value)
                      const length = parseFloat(e.target.value) || 0
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">रुंदी (मीटर) *</label>
                  <Field
                    type="number" step="0.1" name="width"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    onChange={(e) => {
                      formik.setFieldValue('width', e.target.value)
                      const width = parseFloat(e.target.value) || 0
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">क्षेत्रफळ (चौ. मी.)</label>
                  <Field type="number" step="0.01" name="area_sq_m" readOnly className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" placeholder="Auto-calculated" />
                </div>
                ...
                <div>
                  ...
                  <DatePicker
                    selected={formik.values.residency_since}
                    onChange={(date) => { formik.setFieldValue("residency_since", date) }}
                    dateFormat="dd-MM-yyyy"
                    placeholderText="dd-mm-yyyy"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    showMonthDropdown
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="mb-6 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">कुटुंबातील सदस्यांची संख्या *</label>
              <Field
                type="number" name="num_family_members" min="1" max="6" value={displayedMembers}
                onChange={(e) => {
                  const value = parseInt(e.target.value)
                  if (value >= 1 && value <= 6) {
                    setDisplayedMembers(value)
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                ...
                { name: 'doc_front_view', label: 'Front View *', accept: 'image/*', icon: '🏗️' },
                ...[{ name: 'doc_before_2000', label: 'Hut Owner Document Before 2000 Proof', accept: 'image/*,.pdf,.doc,.docx', icon: '', multiple: true }],
                ...[{ name: 'after_2000_proof_submitted', label: 'Hut Owner Document After 2000 Proof', accept: 'image/*,.pdf,.doc,.docx', icon: '📄', multiple: true }],
                { name: 'Seldeclaration_letter', label: 'Self Declaration - A', accept: 'image/*,.pdf', icon: '✍️' },
                { name: 'Ration_card_info', label: 'Self-Declaration Form for Self-Assessment - B', accept: 'image/*,.pdf', icon: '📁' },
                { name: 'sale_agreement', label: 'Sale Agreement', accept: '.pdf,.doc,.docx,image/*', icon: '📜', multiple: true },
                { name: 'biometric', label: 'Biometric Photo', accept: '.pdf,.doc,.docx,image/*', icon: '📜', multiple: true },
                ...
                <div key={name} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-all overflow-hidden">
                  ...
                  {accept?.includes("image") && (
                    <button type="button" onClick={() => setActiveCamera(name)} className="mt-2 w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700">
                      Open Camera
                    </button>
                  )}
                  ...
                  {accept?.includes("video") && recordingField === name && (
                    <video ref={(el) => (videoRefs.current[name] = el)} autoPlay muted className="mt-3 w-full rounded-lg border" />
                  )}
                  ...
                  {activeCamera === name && (
                    <div className="mt-4 p-4 border rounded-lg bg-gray-100">
                      <Webcam
                        ...
                        className="w-full rounded-lg"
                        videoConstraints={{
                          facingMode: /Mobi|Android/i.test(navigator.userAgent) ? { exact: "environment" } : "user"
                        }}
                        onUserMediaError={(err) => {
                          console.error("Camera Error:", err)
                          alert("Camera Error: " + err.message)
                      ...
                      />
                      <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        <button type="button" onClick={() => capturePhoto(name)} className="w-full sm:w-auto bg-green-600 text-white px-4 py-2 rounded-lg">Capture</button>
                        <button type="button" onClick={() => stopWebcamStream()} className="w-full sm:w-auto bg-red-500 text-white px-4 py-2 rounded-lg">Cancel</button>
                      </div>
                    </div>
                  )}
                ...
                <Form>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-10 pt-8 border-t border-gray-200">
                    ...
                    <button
                      ...
                      onClick={prevStep}
                      disabled={currentStep === 1}
                      className={`w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-3 rounded-xl font-semibold ${currentStep === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-200 text-gray-700'}`}
                    >
                      <ChevronLeft size={18} /> Previous
                    </button>
                    ...
                    {isFinalBySurvey ? (
                      ...
                      >
                        {loading ? (
                          <><svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg>Submitting...</>
                        ) : (
                          <><Save size={18} /> Submit</>
                        )}
                      </button>
